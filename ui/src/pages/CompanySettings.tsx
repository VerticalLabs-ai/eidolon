...
export function CompanySettings() {
  ...
  useEffect(() => {
    if (company) {
      ...
      setAlertThreshold(String((company.settings?.alertThresholdPercent as number) ?? 80));
    }
  }, [company]);
    ...
    <PageTransition>
      <div className="mx-auto max-w-2xl p-6 lg:p-8 space-y-8">
        <div>
          ...
          <p className="text-sm text-text-secondary mt-1">Manage company configuration</p>
        </div>

        {/* Navigation to Members & Roles */}
        <Link
          to={`/company/${companyId}/settings/members`}
          className="glass rounded-xl px-6 py-4 flex items-center gap-3 transition-all duration-200 hover:glass-hover group"
        >
          <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-accent/10 shrink-0">
            <Users className="h-4 w-4 text-accent" />
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-sm font-medium text-text-primary">Members &amp; Roles</p>
            <p className="text-xs text-text-secondary mt-0.5">
              View and manage company members, roles, and permissions
            </p>
          </div>
            ...
            <div className="glass-raised rounded-xl p-4 space-y-3">
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs font-semibold text-text-primary tabular-nums font-display">
                  ${((company?.budgetMonthlyCents ?? 0) / 100).toLocaleString()}
                </span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-xs text-text-secondary">Spent</span>
                <span className="text-xs font-semibold text-text-primary tabular-nums font-display">
                  ${((company?.spentMonthlyCents ?? 0) / 100).toLocaleString()}
                </span>
              </div>
              <div className="flex items-center justify-between">
                  ...
                  style={{
                    width: `${Math.min(
                      ...
                      company?.budgetMonthlyCents
                        ? ((company.spentMonthlyCents ?? 0) / company.budgetMonthlyCents) * 100
                        : 0,
                    )}%`,
                  }}
                />
        ...
        <div className="glass rounded-xl overflow-hidden">
          ...
          <div className="p-6 space-y-4">
            {/* Add key form */}
            {showAddKey && (
              <form onSubmit={handleAddKey} className="rounded-xl glass-raised p-5 space-y-4">
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  <Input
                    label="Key Name"
                ...
                <div className="flex items-center justify-end gap-3 pt-1">
                  ...
                  <Button type="submit" size="sm" loading={createSecret.isPending}>
                    Save Key
                  </Button>
                </div>
              </form>
            ...
            {!secrets?.length && !showAddKey ? (
              <div className="py-8 text-center">
                ...
                <p className="text-sm text-text-secondary">No API keys configured yet.</p>
        ...
        <div
          ...
          style={{ background: 'rgba(255,68,102,0.03)' }}
        >
          ...
          <div className="p-6 space-y-5">
            <div className="flex items-center justify-between gap-4">
              <div>
                <p className="text-sm font-medium text-text-primary">Pause Company</p>
                <p className="text-xs text-text-secondary mt-0.5">
              ...
              <Button
                ...
                onClick={() =>
                  handleStatusChange(company?.status === 'paused' ? 'active' : 'paused')
                }
              >
            ...
            <div className="flex items-center justify-between gap-4">
              <div>
                <p className="text-sm font-medium text-text-primary">Archive Company</p>
                <p className="text-xs text-text-secondary mt-0.5">
