      ...
      <li key={node.id} role="treeitem" aria-expanded={hasChildren ? isExpanded : undefined} aria-selected={isSelected}>
        <div
          className={clsx(
            ...
            isSelected ? "bg-accent/10 text-text-primary" : "hover:bg-white/[0.03] text-text-secondary",
          )}
        ...
        >
          ...
          >
            {isExpanded ? <FolderOpen className="h-3.5 w-3.5 shrink-0 text-accent/70" /> : <FolderIcon className="h-3.5 w-3.5 shrink-0 text-accent/70" />}
            {renamingId === node.id ? (
              <span className="flex flex-1 items-center gap-1">
                ...
                />
                <button onClick={() => void handleRename(node.id)} className="text-accent hover:text-neon-cyan" aria-label="Confirm rename">
                  <Check className="h-3 w-3" />
                </button>
                <button onClick={() => { setRenamingId(null); setRenameValue(""); }} className="text-text-secondary hover:text-error" aria-label="Cancel rename">
                  <X className="h-3 w-3" />
                </button>
              </span>
            ) : (
        ...
        {creatingUnder === node.id && (
          <div className="flex items-center gap-1 py-1" style={{ paddingLeft: (depth + 1) * 12 + 4 }}>
            ...
            <input
              ...
              onKeyDown={(e) => {
                ...
                if (e.key === "Escape") { setCreatingUnder(undefined); setNewName(""); }
              }}
            ...
            />
            <button onClick={() => void handleCreate(node.id)} className="text-accent hover:text-neon-cyan" aria-label="Confirm create">
              <Check className="h-3 w-3" />
            </button>
            <button onClick={() => { setCreatingUnder(undefined); setNewName(""); }} className="text-text-secondary hover:text-error" aria-label="Cancel create">
              <X className="h-3 w-3" />
            </button>
          </div>
        )}
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="flex-1 overflow-y-auto px-1 pb-2">
        ...
        {creatingUnder === null && (
          <div className="flex items-center gap-1 px-2 py-1">
            ...
            <input
              ...
              className="w-full rounded border border-accent/40 bg-surface px-1.5 py-0.5 text-xs text-text-primary outline-none placeholder:text-text-secondary/40"
            />
            <button onClick={() => void handleCreate(null)} className="text-accent hover:text-neon-cyan" aria-label="Confirm create">
              <Check className="h-3 w-3" />
            </button>
            <button onClick={() => { setCreatingUnder(undefined); setNewName(""); }} className="text-text-secondary hover:text-error" aria-label="Cancel create">
              <X className="h-3 w-3" />
        ...
        {folders.length === 0 && creatingUnder === undefined && (
          <p className="px-3 py-2 text-xs text-text-secondary/60">No folders yet. Click + to create one.</p>
        )}
      </div>
      ...
      >
        <div className="space-y-4">
          <p className="text-sm text-text-secondary">
            Delete folder <span className="font-semibold text-text-primary">"{deleteTarget?.name}"</span>?
            Its contents (folders and artifacts) will be moved to the parent
            folder (or unfiled if top-level). No artifacts will be deleted.
          </p>
...
export function FolderBreadcrumbs({
  ...
  return (
    <nav aria-label="Folder path" className="flex min-w-0 flex-wrap items-center gap-1 text-xs text-text-secondary">
      <button
