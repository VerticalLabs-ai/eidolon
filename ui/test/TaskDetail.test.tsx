...
vi.mock('@/lib/hooks', () => ({
  useTaskMeetings: () => ({ data: [], isLoading: false, isError: false }),
...
describe('TaskDetail', () => {
  ...
  it('shows thread read failures and lets the operator retry', async () => {
    ...
    expect(
      screen.queryByText('No comments, approvals, or execution events yet.'),
  ...
  it('focuses linked thread items and marks inbox source read', () => {
    ...
    const focusedItem = screen
      .getByText('Need operator direction')
      .closest('#thread-interaction-1');
