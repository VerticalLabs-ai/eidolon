...
import { useCreateGoal, useUpdateGoal } from "../src/lib/hooks";
...
describe("goal mutations", () => {
  ...
  beforeEach(() => {
    ...
    queryClient.setQueryData(["goals", "company-1"], []);
    wrapper = ({ children }) => (
  ...
  it("adds the canonical created goal and invalidates the tree", async () => {
    ...
    expect(queryClient.getQueryData(["goals", "company-1"])).toEqual([rootGoal]);
    expect(queryClient.getQueryState(["goals", "company-1"])?.isInvalidated).toBe(true);
  });
  ...
  it("replaces the updated goal and invalidates the tree", async () => {
    ...
    queryClient.setQueryData(["goals", "company-1"], [rootGoal]);
    ...
    expect(queryClient.getQueryData(["goals", "company-1"])).toEqual([updatedGoal]);
    expect(queryClient.getQueryState(["goals", "company-1"])?.isInvalidated).toBe(true);
  });
});
