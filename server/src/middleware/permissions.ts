...
/**
 * Convert the numeric role levels used by legacy authorization code into the
 * formal RBAC role names. Unknown levels are safest as read-only.
 */
export function getRoleFromPermissionLevel(level: number | string): Role {
  if (level === 'owner' || level === 'admin' || level === 'member' || level === 'viewer') {
    return level;
  }
