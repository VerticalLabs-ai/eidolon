...
describe('desktop navigation policy', () => {
  it('defaults to the production app', () => {
    expect(resolveAppUrl().toString()).toBe('https://eidolon.verticallabs.ai/');
  });
  ...
  it('keeps Clerk and Google auth redirects inside the desktop window', () => {
    ...
    expect(
      shouldKeepNavigationInApp('https://accounts.eidolon.verticallabs.ai/sign-in', {
        allowedHosts,
        authFlowHosts,
      }),
    ).toBe(true);
  ...
  it('rejects non-https app URLs outside loopback development', () => {
    expect(() => resolveAppUrl('http://eidolon.verticallabs.ai')).toThrow('https URL');
  });
});
